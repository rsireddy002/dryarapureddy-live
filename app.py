...
if os.path.exists(CACHE_PATH):
    ...
    tab_scanner, tab_levels, tab_chart, tab_sectors, tab_rvol, tab_range, tab_breakout, tab_zonewatch, tab_replay, tab_alerts, tab_liveticks = st.tabs(
        ["Scanner", "Key Levels", "Chart", "Sectors", "By RVOL", "Wide Range", "Breakout Watch", "Zone Watch", "Replay", "Alerts", "Live Ticks"]
    )
        ...
        if not symbols_with_zones:
            ...
            if replay_mode == "Single symbol":
                ...
                if day_df is None or day_df.empty:
                    ...
                    _replay_padding = (so_far["high"].max() - so_far["low"].min()) * 0.05
                    _replay_y_range = (so_far["low"].min() - _replay_padding, so_far["high"].max() + _replay_padding)
                    fig = plot_candles_with_zones(
                        so_far,
                        composite_zones=composite_zones,
                        intraday_zones=intraday_zones,
